...
'''
from https://github.com/Yushi-Hu/IC-DST/
'''
    ...
    for domain in domains:
        ...
        for slot in bstate[domain]['semi']:
            ...
            if bstate[domain]['semi'][slot] == 'not mentioned':
                ...
                # unify ontology of 2.1 and 2.4: conerthall, nightclub
                normalized_value = normalize(bstate[domain]['semi'][slot].strip().lower(), False)
                if normalized_value == 'night club':
                    normalized_value = 'nightclub'
                if normalized_value == 'concerthall':
                    normalized_value = 'concert hall'
                summary_bvalue.append(["{}-{}".format(domain, slot.strip().lower()), normalized_value]) #(["semi", domain, slot, bstate[domain]['semi'][slot]])
            if slot_enc != [0, 0, 0]:
                domain_active = True
...
def divideData(data,args):
    ...
    with open(os.path.join(args.target_path, 'ontology.json'), 'w') as f:
        json.dump(get_ontology(val_dials + test_dials), f, indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--main_dir", type=str, default='data/mw24_src')
    parser.add_argument("--mwz_ver", type=str, default='2.4')
    parser.add_argument("--target_path", type=str, default='data/mw24')
